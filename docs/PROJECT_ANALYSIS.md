...
| 좋아요·태그 | 곡 행의 음표 아이콘은 좋아요를 Room에 저장합니다. 좋아요 필터는 현재 목록에서 좋아요 곡만 남깁니다. 태그 편집 화면은 느낌과 상황 태그를 저장합니다. |
...
| Lounge | Firebase 익명 인증 후 Firestore 게시글을 실시간으로 봅니다. 제목·가수·태그·링크를 넣어 공유하고 게시글을 신고할 수 있습니다. 관리자 계정에는 삭제·사용자 차단 메뉴가 보입니다. |
...
- Lounge 게시글의 `Listen` 버튼은 클릭 동작이 비어 있습니다. 재생목록 삭제는 서비스 메서드만 있고 홈 화면에서 호출하지 않습니다. 재생목록 종료 이벤트도 화면에 표시하는 대화상자가 없습니다.
